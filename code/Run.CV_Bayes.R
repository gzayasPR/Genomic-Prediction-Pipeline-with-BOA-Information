...
print(paste("Running",model.fit,"model"))
...
for (k in 1:K) {
  ...
  ETA_G_B_OMEGA <- list(FIXED = list(~factor(Set), data = phenotypes, model = 'FIXED'),
                        G =  list(X = X, model = model.fit),
                        B = list(X = O, model = model.fit),
                        OMEGA = list(X = O_X, model = model.fit))
  ...
  results[[k]] <- list(
    ...
    var_G_B_OMEGA = c((fm_G_B_OMEGA$ETA$G$varU + fm_G_B_OMEGA$ETA$B$varU + fm_G_B_OMEGA$ETA$OMEGA$varU) / 
                      ...
                      fm_G_B_OMEGA$ETA$G$varU,fm_G_B_OMEGA$ETA$B$varU,fm_G_B_OMEGA$ETA$OMEGA$varU,fm_G_B$varE))
  setwd(base_dir)
}
...
write.csv(results_summary, paste(model.fit, "results_summary.csv"), row.names = FALSE)
...
closeAllConnections()
setwd(base_dir)
write.csv(summary_table,paste(model.fit,"Variance Componenets.csv"))
setwd(code_dir)
